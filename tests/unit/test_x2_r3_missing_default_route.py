...
from src.collection.default_route_state_collector import (
    collect_missing_default_route_evidence_v4,
)
...
from src.fault_injection.missing_default_route import (
    inject_missing_default_route,
    restore_missing_default_route,
)
from src.orchestration.x2_missing_default_route_experiment_runner import run_x2_r3_experiment
from src.rules.addressing_rule_engine_x2_r3 import diagnose_addressing_v2


ROOT = Path(__file__).resolve().parents[2]
SCENARIO = ROOT / "scenarios/expansion/X2_R3_MISSING_DEFAULT_ROUTE.yml"
...
class FakeNetwork:
    ...
    def __call__(
        ...
        if parts[:1] == ["ping"]:
            target = parts[-1]
            reachable = target == "10.20.1.1" or self.route_present
            return self._result(
                container,
                command,
                return_code=0 if reachable else 2,
                stderr="" if reachable else "ping: connect: Network is unreachable\n",
            )
        ...
        if parts[:3] == ["sh", "-eu", "-c"]:
            shell = parts[3]
            if "ip route del default" in shell:
            ...
            if "ip route replace default" in shell:
                self.prefix_length = 24
                self.route_present = True
                return self._result(container, command)
...
def _fault_vector(tmp_path: Path, network: FakeNetwork) -> dict[str, object]:
    inject_missing_default_route(SCENARIO, tmp_path / "mutation", executor=network)
    evidence = collect_missing_default_route_evidence_v4(
        tmp_path, SCENARIO, executor=network
    )
    return build_feature_vector_v2(tmp_path, evidence)


def test_scenario_changes_only_default_route_state() -> None:
    binding = load_missing_default_route_scenario(SCENARIO)
...
def test_injection_and_restoration_are_exact_and_idempotent(tmp_path: Path) -> None:
    ...
    injection = inject_missing_default_route(SCENARIO, mutation, executor=network)
    assert injection["status"] == "FAULT_CONFIRMED"
    assert network.address == "10.20.1.10"
    assert network.prefix_length == 24
    assert network.route_present is False
    restoration = restore_missing_default_route(SCENARIO, mutation, executor=network)
    assert restoration["status"] == "RESTORATION_CONFIRMED"
    assert network.prefix_length == 24
    assert network.route_present is True
    assert restore_missing_default_route(SCENARIO, mutation, executor=network) == restoration
...
def test_exception_after_real_mutation_triggers_best_effort_restoration(
    ...
    with pytest.raises(Exception, match="executor raised an exception"):
        inject_missing_default_route(SCENARIO, mutation, executor=network)
    assert network.prefix_length == 24
    assert network.route_present is True
...
def test_restorer_rejects_unjournaled_calls(tmp_path: Path) -> None:
    with pytest.raises(X2AddressingError, match="durable recovery intent"):
        restore_missing_default_route(SCENARIO, tmp_path / "mutation", executor=FakeNetwork())
...
def test_native_evidence_v4_has_exact_signature_and_hashes(tmp_path: Path) -> None:
    ...
    assert values["source_prefix_matches_expected"]["value"] is True
    assert values["source_default_route_present"]["value"] is False
    ...
    restore_missing_default_route(SCENARIO, tmp_path / "mutation", executor=network)


def test_combined_rule_diagnoses_missing_default_route(tmp_path: Path) -> None:
    network = FakeNetwork()
    vector = _fault_vector(tmp_path, network)
    ...
    assert diagnosis["prediction"]["fault_type"] == "missing_default_route"
    assert diagnosis["explanation_refs"] == ["rule:R_X2_ADDRESSING_003"]
    restore_missing_default_route(SCENARIO, tmp_path / "mutation", executor=network)
...
def test_combined_rule_preserves_x2_r1_wrong_ip_signature(tmp_path: Path) -> None:
    ...
    previous["values"]["source_default_route_present"]["value"] = True
    diagnosis = diagnose_addressing_v2(
...
def test_combined_rule_abstains_on_unreviewed_confounder(tmp_path: Path) -> None:
    ...
    conflicting["values"]["source_address_matches_expected"]["value"] = False
    diagnosis = diagnose_addressing_v2(
...
def test_orchestrator_diagnoses_then_restores_baseline(tmp_path: Path) -> None:
    ...
    def baseline(_: Path) -> dict[str, object]:
        return {
            "command": ["bash", "fake"],
            "return_code": 0 if network.prefix_length == 24 and network.route_present else 1,
            "stdout": "",
            "stderr": "",
            "timestamp_utc": utc_now(),
        }

    result = run_x2_r3_experiment(
        SCENARIO,
        tmp_path / "experiments",
        ROOT / "unused.sh",
        baseline_validator=baseline,
        fault_injector=lambda scenario, output: inject_missing_default_route(
            scenario, output, executor=network
        ),
        fault_restorer=lambda scenario, output: restore_missing_default_route(
            scenario, output, executor=network
        ),
        evidence_collector=lambda output, scenario: collect_missing_default_route_evidence_v4(
            output, scenario, executor=network
        ),
        experiment_id="x2-r3-unit-cycle",
    )
