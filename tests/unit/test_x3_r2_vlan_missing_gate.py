...
from src.expansion import x3_r2_gate
from src.expansion.x3_r2_gate import (
    EXPECTED_RUNTIME,
    EXPECTED_SAFETY,
    EXPECTED_SIGNATURE,
    X3R2GateError,
    validate_x3_r2_manifest,
    verify_x3_r2_gate,
)


ROOT = Path(__file__).resolve().parents[2]
MANIFEST = ROOT / "plans/expansion/X3_R2_VLAN_MISSING_V1.json"
SCHEMA = ROOT / "schemas/x3_r2_vlan_missing_gate_v1.schema.json"
...
def test_schema_and_repository_gate_verify() -> None:
    ...
    manifest = verify_x3_r2_gate(ROOT)
    assert manifest["status"] == "IMPLEMENTED_RUNTIME_SLICE"
    assert manifest["track"]["next_release"] == "X3_R3_VLAN_NOT_ALLOWED_ON_TRUNK"
...
def test_collector_and_acceptance_boundaries_are_explicit() -> None:
    ...
    assert collector["collector_version"] == 2
...
def test_semantic_gate_rejects_boundary_drift(mutation: str) -> None:
    ...
    with pytest.raises(X3R2GateError):
        validate_x3_r2_manifest(manifest, _load(SCHEMA))
...
def test_repository_gate_rejects_source_hash_drift(
    ...
    original = x3_r2_gate._load

    def load(path: Path):
        if path == ROOT / x3_r2_gate.MANIFEST_PATH:
            return manifest
        return original(path)

    monkeypatch.setattr(x3_r2_gate, "_load", load)
    with pytest.raises(X3R2GateError, match="source binding drifted"):
        verify_x3_r2_gate(ROOT)
...
def test_runtime_modules_import_no_ml_dataset_or_report_code() -> None:
    ...
    paths = (
        "src/expansion/x3_vlan_missing.py",
        "src/fault_injection/vlan_missing.py",
        "src/collection/l2_vlan_state_collector_v2.py",
        "src/rules/l2_vlan_rule_engine_x3_r2.py",
        "src/orchestration/x3_vlan_missing_experiment_runner.py",
    )
...
def test_real_lifecycle_is_explicit_opt_in() -> None:
    text = (
        ROOT / "tests/e2e/test_x3_r2_vlan_missing_containerlab.py"
    ).read_text(encoding="utf-8")
    assert "IND_RUN_X3_R2_E2E" in text
