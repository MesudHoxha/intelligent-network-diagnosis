...
from src.expansion import x3_r1_gate
from src.expansion.x3_r1_gate import (
    EXPECTED_RUNTIME,
    EXPECTED_SAFETY,
    EXPECTED_SIGNATURE,
    X3R1GateError,
    validate_x3_r1_manifest,
    verify_x3_r1_gate,
)


ROOT = Path(__file__).resolve().parents[2]
MANIFEST = ROOT / "plans/expansion/X3_R1_WRONG_ACCESS_VLAN_V1.json"
SCHEMA = ROOT / "schemas/x3_r1_wrong_access_vlan_gate_v1.schema.json"
...
def test_schema_and_repository_gate_verify() -> None:
    ...
    assert manifest["track"]["next_release"] == "X3_R2_VLAN_MISSING"
...
def test_semantic_gate_rejects_boundary_drift(mutation: str) -> None:
    ...
    if mutation == "runtime":
        ...
        manifest["slice"]["signature"]["vlan_allowed_on_trunk"] = False
    elif mutation == "safety":
        manifest["safety"]["invariants"].pop()
    else:
        manifest["acceptance"]["dataset_row_created"] = True
    with pytest.raises(X3R1GateError):
        validate_x3_r1_manifest(manifest, _load(SCHEMA))
...
def test_repository_gate_rejects_source_hash_drift(
    ...
    original = x3_r1_gate._load

    def load(path: Path):
        if path == ROOT / x3_r1_gate.MANIFEST_PATH:
            return manifest
        return original(path)

    monkeypatch.setattr(x3_r1_gate, "_load", load)
    with pytest.raises(X3R1GateError, match="source binding drifted"):
        verify_x3_r1_gate(ROOT)
...
def test_runtime_modules_import_no_ml_dataset_or_report_code() -> None:
    ...
    paths = (
        "src/expansion/x3_wrong_access_vlan.py",
        "src/fault_injection/wrong_access_vlan.py",
        "src/collection/l2_vlan_state_collector.py",
        "src/rules/l2_vlan_rule_engine_v2.py",
        "src/orchestration/x3_wrong_access_vlan_experiment_runner.py",
    )
...
def test_real_lifecycle_is_explicit_opt_in() -> None:
    text = (
        ROOT / "tests/e2e/test_x3_r1_wrong_access_vlan_containerlab.py"
    ).read_text(encoding="utf-8")
    assert "IND_RUN_X3_R1_E2E" in text
