from __future__ import annotations

import copy
import json
from pathlib import Path

import pytest

from src.expansion.x4_r6_gate import X4R6CloseoutError, verify_x4_r6_receipt, verify_x4_r6_source_gate

ROOT = Path(__file__).resolve().parents[2]
PLAN = ROOT / "plans/expansion/X4_R6_DHCP_DNS_SERVICE_SECURITY_CLOSEOUT_V1.json"
RECEIPT = ROOT / "plans/expansion/X4_R6_DHCP_DNS_SERVICE_SECURITY_EVIDENCE_RECEIPT_V1.json"

def test_closeout_binds_exact_disjoint_d1_through_d5_slices() -> None:
    plan = verify_x4_r6_source_gate(ROOT)
    assert plan["status"] == "ACCEPTED_SOURCE_CLOSEOUT"
    assert plan["track"]["next_release"] == "P9_R1_THESIS_WRITING_AND_DEFENSE"
    assert len(plan["accepted_slices"]) == 5
    assert len({json.dumps(row["expected_signature"], sort_keys=True) for row in plan["accepted_slices"]}) == 5
    assert plan["d3_release_alias"]["mapping"] == "ONE_TO_ONE_SAME_SLICE"

def test_closeout_authorizes_no_runtime_or_scientific_operation() -> None:
    runtime = verify_x4_r6_source_gate(ROOT)["runtime_authorization"]
    assert len(runtime) == 10 and all(value is False for value in runtime.values())

def test_receipt_reverifies_five_materialized_revalidation_runs() -> None:
    receipt = verify_x4_r6_receipt(RECEIPT, repository_root=ROOT, verify_materialized=True)
    assert receipt["evidence_kind"] == "REPRODUCIBILITY_REVALIDATION_NOT_ORIGINAL_ACCEPTANCE_ARCHIVE"
    assert receipt["summary"]["run_count"] == 5
    assert receipt["summary"]["all_raw_hashes_verified"] is True

def test_source_gate_rejects_runtime_or_signature_drift(monkeypatch: pytest.MonkeyPatch) -> None:
    plan = json.loads(PLAN.read_text(encoding="utf-8")); changed = copy.deepcopy(plan); changed["runtime_authorization"]["metric_calculation"] = True
    import src.expansion.x4_r6_gate as gate
    original = gate._load
    monkeypatch.setattr(gate, "_load", lambda path, label: changed if path == ROOT / gate.PLAN else original(path, label))
    with pytest.raises(X4R6CloseoutError): verify_x4_r6_source_gate(ROOT)
