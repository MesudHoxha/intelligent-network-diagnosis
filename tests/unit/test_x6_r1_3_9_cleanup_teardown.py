...
import pytest
from src.orchestration.x6_r1_3_9_contract import ROOT, PREDECESSOR, Invalid, canonical, digest, frozen_contract, identity, load, validate_authorization
from src.orchestration.x6_r1_3_9_durable import reserve, reservation_path
...
def environment(tmp_path, *, fail=False):
    ...
    for name in ("containerlab", "docker", "ip", "tc", "ps", "uname", "zgrep", "lsmod", "modinfo", "python3", "ethtool", "git"):
        (bin_path / name).symlink_to(ROOT / "tests/fixtures/x6_r1_3_9_external_stub.py")
...
def command(auth, root):
    return [sys.executable, "-m", "src.orchestration.x6_r1_3_9_production_path", "--authorization", str(auth), "--run-root", str(root), "--run-id", "synthetic-run", "--execute", "--source-test"]
...
def test_cli_command_failure_terminalizes_and_standalone_recovery_accepts_failed_records(tmp_path):
    ...
    recovery=subprocess.run([sys.executable,"-m","src.orchestration.x6_r1_3_9_recovery","--run-root",str(root),"--source-test"],env=env,cwd=ROOT,capture_output=True,text=True,timeout=30)
...
def test_cli_abrupt_death_is_recovered_from_actual_intermediate_state(tmp_path):
    ...
    try:
        ...
        result=subprocess.run([sys.executable,"-m","src.orchestration.x6_r1_3_9_recovery","--run-root",str(root),"--source-test"],env=env,cwd=ROOT,capture_output=True,text=True,timeout=30)
...
def complete_simulated_run(tmp_path_factory):
    base=tmp_path_factory.mktemp("r139-complete-simulated")
    ...
    assert result.returncode == 0,result.stderr
    replay=subprocess.run([sys.executable,"-m","src.orchestration.x6_r1_3_9_recovery","--run-root",str(root),"--source-test"],env=env,cwd=ROOT,capture_output=True,text=True,timeout=60)
    assert replay.returncode == 0,replay.stderr
    return root,env


def test_complete_cli_and_standalone_replay_reconstruct_all_thirty_windows(complete_simulated_run):
    from src.expansion.x6_r1_3_9_materialized_verifier import verify
...
def test_successful_replay_reconstructs_owned_cleanup_before_and_clean_after(complete_simulated_run):
    from src.orchestration.x6_r1_3_9_durable import records
    from src.orchestration.x6_r1_3_9_observations import catalog, reconstruct_cleanup_cycles
    ...
    cycles = reconstruct_cleanup_cycles(
        ...
        release_id="X6_R1_3_9_CLEANUP_AND_TEARDOWN_OBSERVATION_CORRECTION", successful=True,
    )
...
def _recorder_environment(tmp_path, monkeypatch, **state_values):
    from src.orchestration.x6_r1_3_9_durable import Recorder
    from src.orchestration.x6_r1_3_9_observations import catalog
...
def test_teardown_accepts_proved_absence_and_successful_owned_stop(tmp_path, monkeypatch, running, expected_count):
    from src.orchestration.x6_r1_3_9_observations import failed_command_orders
    from src.orchestration.x6_r1_3_9_production_path import teardown
...
def test_successor_bindings_and_historical_preservation():
    from src.expansion.x6_r1_3_9_gate import verify_source
