...
from src.collection.l2_vlan_state_collector_v2 import (
    build_l2_vlan_feature_vector_v2,
    collect_vlan_missing_evidence_v4,
)
...
from src.fault_injection.vlan_missing import (
    inject_vlan_missing,
    restore_vlan_missing,
)
from src.orchestration.x3_vlan_missing_experiment_runner import (
    recover_x3_r2_experiment,
    run_x3_r2_experiment,
)
from src.rules.l2_vlan_rule_engine_x3_r2 import diagnose_l2_vlan_x3_r2_v2


ROOT = Path(__file__).resolve().parents[2]
SCENARIO = ROOT / "scenarios/expansion/X3_R2_VLAN_MISSING.yml"
...
class FakeNetwork:
    ...
    def vlan_rows(self, container: str) -> list[dict[str, object]]:
        missing = self.faulted and container.endswith("sw1")
        return [
            {
                "ifname": "eth1",
                "vlans": []
                if missing
                else [{"vlan": 10, "flags": ["PVID", "Egress Untagged"]}],
            },
            {
                "ifname": "eth2",
                "vlans": [
    ...
    def __call__(self, container: str, command: Sequence[str]) -> dict[str, object]:
        ...
        if parts[:4] == ["bridge", "-j", "fdb", "show"]:
            rows = (
                ...
                if container.endswith("sw1") and not self.faulted
                else []
            )
            return self.result(container, command, stdout=json.dumps(rows))
        ...
        if parts[:3] == ["sh", "-eu", "-c"]:
            shell = parts[3]
            if (
                "bridge vlan del dev eth1 vid 10" in shell
                and "bridge vlan del dev eth3 vid 10" in shell
                and "bridge vlan add" not in shell
            ):
                self.faulted = True
...
def _fault_evidence(tmp_path: Path, network: FakeNetwork) -> dict[str, object]:
    inject_vlan_missing(SCENARIO, tmp_path / "mutation", executor=network)
    return collect_vlan_missing_evidence_v4(tmp_path, SCENARIO, executor=network)


def test_scenario_binds_exact_tagged_flow_and_vlan_identity() -> None:
    binding = load_vlan_missing_scenario(SCENARIO)
    ...
    assert binding.affected_resource == "br0:vlan10"
...
def test_injection_is_effective_and_restoration_is_idempotent(tmp_path: Path) -> None:
    ...
    injection = inject_vlan_missing(SCENARIO, mutation, executor=network)
    ...
    restoration = restore_vlan_missing(SCENARIO, mutation, executor=network)
    assert restoration["status"] == "RESTORATION_CONFIRMED"
    assert network.faulted is False
    assert restore_vlan_missing(SCENARIO, mutation, executor=network) == restoration
...
def test_crash_after_mutation_restores_from_durable_intent(tmp_path: Path) -> None:
    ...
    with pytest.raises(Exception, match="executor raised an exception"):
        inject_vlan_missing(SCENARIO, mutation, executor=network)
    assert network.faulted is False
...
def test_unjournaled_restoration_is_rejected(tmp_path: Path) -> None:
    with pytest.raises(X3VlanMissingError, match="durable recovery intent"):
        restore_vlan_missing(SCENARIO, tmp_path / "mutation", executor=FakeNetwork())


def test_evidence_has_exact_vlan_missing_signature_and_hashes(tmp_path: Path) -> None:
    network = FakeNetwork()
    ...
    restore_vlan_missing(SCENARIO, tmp_path / "mutation", executor=network)
...
def test_vector_and_rule_diagnose_only_exact_signature(tmp_path: Path) -> None:
    ...
    diagnosis = diagnose_l2_vlan_x3_r2_v2(
        vector, location_node="sw1", affected_resource="br0:vlan10"
    )
    assert diagnosis["status"] == "diagnosed"
    assert diagnosis["prediction"] == {
        "fault_type": "vlan_missing",
        "score": 1.0,
        "location": "sw1",
        "affected_resource": "br0:vlan10",
    }
    assert diagnosis["explanation_refs"] == ["rule:R_X3_L2_VLAN_002"]
    changed = copy.deepcopy(vector)
    changed["values"]["vlan_exists_on_target"]["value"] = True
    changed["values"]["vlan_allowed_on_trunk"]["value"] = True
    preserved = diagnose_l2_vlan_x3_r2_v2(
        changed, location_node="sw1", affected_resource="eth1"
    )
    assert preserved["prediction"]["fault_type"] == "wrong_access_vlan"
    assert preserved["explanation_refs"] == ["rule:R_X3_L2_VLAN_001"]
    changed["values"]["native_vlan_matches_peer"]["value"] = False
    assert diagnose_l2_vlan_x3_r2_v2(
        changed, location_node="sw1", affected_resource="eth1"
    )["status"] == "abstained"
    restore_vlan_missing(SCENARIO, tmp_path / "mutation", executor=network)
...
def test_orchestrator_completes_restores_and_can_replay_recovery(tmp_path: Path) -> None:
    ...
    result = run_x3_r2_experiment(
        SCENARIO,
        tmp_path / "runs",
        ROOT / "unused.sh",
        baseline_validator=baseline,
        fault_injector=lambda scenario, output: inject_vlan_missing(
            scenario, output, executor=network
        ),
        fault_restorer=lambda scenario, output: restore_vlan_missing(
            scenario, output, executor=network
        ),
        evidence_collector=lambda output, scenario: collect_vlan_missing_evidence_v4(
            output, scenario, executor=network
        ),
        experiment_id="x3-r2-unit-cycle",
    )
    assert result["status"] == "COMPLETED"
    assert result["restoration_confirmed"] is True
    assert result["baseline_valid_after"] is True
    assert network.faulted is False
    recovered = recover_x3_r2_experiment(
        SCENARIO,
...
def test_existing_mutation_output_is_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(X3VlanMissingError, match="already exists"):
        inject_vlan_missing(SCENARIO, mutation, executor=FakeNetwork())
