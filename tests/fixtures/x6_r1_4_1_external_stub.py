...
with state_path.open("r+") as stream:
    ...
    if name == "containerlab" and args[0] in {"deploy", "destroy"}:
        ...
        if args[0] == "ps":
            out = "\n".join(json.dumps({"Names": "clab-x6r1-" + node, "ID": state.get("replacement_id", {}).get(node, node), "Image": "ind-linux:0.1", "Labels": "containerlab=x6r1"}) for node in ("hosta", "r1", "r2", "r3", "hostb")) if state.get("deployed") else ""
