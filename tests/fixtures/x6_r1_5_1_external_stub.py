...
fast = os.environ.get("X6_R1_5_1_CONTROLLED_CLOCK") == "1"
    ...
    if name == "containerlab" and args[0] in {"deploy", "destroy"}:
        ...
        if args[0] == "ps":
            ...
            if not state.get("deployed"):
                ...
                bits = 50000000 if state.get("diagnostic_abstention") and state.get("qdisc") == "fault" else 7750000000
                out = json.dumps({"end": {"sum_received": {"bits_per_second": bits}}})
                delay = .05 if fast else 20.05
                state["server_running"] = False
                if state.get("fail_iperf") or (state.get("fail_fault_iperf") and state.get("qdisc") == "fault"): rc, delay = 1, 0
