...
from src.orchestration.x3_vlan_missing_experiment_runner import run_x3_r2_experiment
...
RUN_INFRASTRUCTURE = os.environ.get("IND_RUN_X3_R2_E2E") == "1"

pytestmark = [
    pytest.mark.infrastructure,
    pytest.mark.skipif(
        not RUN_INFRASTRUCTURE,
        reason="set IND_RUN_X3_R2_E2E=1 to run the X3-R2 Containerlab lifecycle",
    ),
]


def test_real_vlan_missing_cycle_diagnoses_restores_and_cleans_up(
    tmp_path: Path,
) -> None:
    ...
    try:
        ...
        require_success(
            ...
            "X3-R2 Containerlab deployment",
        )
        configured_root = os.environ.get("IND_X3_R2_EVIDENCE_ROOT")
        output_root = Path(configured_root) if configured_root else tmp_path / "experiments"
        result = run_x3_r2_experiment(
            SCENARIO,
            output_root,
            BASELINE,
            experiment_id="x3-r2-real-vlan-missing",
        )
        ...
        assert values["vlan_exists_on_target"]["value"] is False
        ...
        assert diagnosis["prediction"]["fault_type"] == "vlan_missing"
        assert diagnosis["prediction"]["location"] == "sw1"
        assert diagnosis["prediction"]["affected_resource"] == "br0:vlan10"
        ...
        if deploy_attempted:
            ...
            if cleanup["return_code"] != 0 and primary_error is None:
                pytest.fail("X3-R2 Containerlab cleanup failed: " + str(cleanup["stderr"]))
        assert containerlab_containers(
