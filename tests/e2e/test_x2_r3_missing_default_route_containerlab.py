...
from src.orchestration.x2_missing_default_route_experiment_runner import (
    run_x2_r3_experiment,
)
...
RUN_INFRASTRUCTURE = os.environ.get("IND_RUN_X2_R3_E2E") == "1"

pytestmark = [
    pytest.mark.infrastructure,
    pytest.mark.skipif(
        not RUN_INFRASTRUCTURE,
        reason="set IND_RUN_X2_R3_E2E=1 to run the X2-R3 Containerlab lifecycle",
    ),
]


def test_real_missing_default_route_cycle_diagnoses_restores_and_cleans_up(
    tmp_path: Path,
) -> None:
    ...
    try:
        ...
        require_success(
            ...
            "X2-R3 Containerlab deployment",
        )
        configured_evidence_root = os.environ.get("IND_X2_R3_EVIDENCE_ROOT")
        output_root = (
        ...
        result = run_x2_r3_experiment(
            ...
            output_root,
            BASELINE,
            experiment_id="x2-r3-real-missing-default-route",
        )
        ...
        if deploy_attempted:
            ...
            if cleanup["return_code"] != 0 and primary_error is None:
                pytest.fail("X2-R3 Containerlab cleanup failed: " + str(cleanup["stderr"]))
        assert containerlab_containers(
            repository_root=ROOT,
            command_executor=run_command,
