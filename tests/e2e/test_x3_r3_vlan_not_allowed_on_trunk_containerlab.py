...
from src.orchestration.x3_vlan_not_allowed_on_trunk_experiment_runner import run_x3_r3_experiment
...
RUN_INFRASTRUCTURE = os.environ.get("IND_RUN_X3_R3_E2E") == "1"

pytestmark = [
    pytest.mark.infrastructure,
    pytest.mark.skipif(
        not RUN_INFRASTRUCTURE,
        reason="set IND_RUN_X3_R3_E2E=1 to run the X3-R3 Containerlab lifecycle",
    ),
]


def test_real_vlan_not_allowed_on_trunk_cycle_diagnoses_restores_and_cleans_up(
    tmp_path: Path,
) -> None:
    ...
    try:
        ...
        require_success(
            ...
            "X3-R3 Containerlab deployment",
        )
        configured_root = os.environ.get("IND_X3_R3_EVIDENCE_ROOT")
        output_root = Path(configured_root) if configured_root else tmp_path / "experiments"
        result = run_x3_r3_experiment(
            SCENARIO,
            output_root,
            BASELINE,
            experiment_id="x3-r3-real-vlan-not-allowed-on-trunk",
        )
        ...
        assert values["access_vlan_matches_expected"]["value"] is True
        assert values["vlan_exists_on_target"]["value"] is True
        assert values["vlan_allowed_on_trunk"]["value"] is False
        assert values["native_vlan_matches_peer"]["value"] is True
        assert values["fdb_location_matches_expected"]["value"] is True
        ...
        assert diagnosis["prediction"]["fault_type"] == "vlan_not_allowed_on_trunk"
        assert diagnosis["prediction"]["location"] == "sw1"
        assert diagnosis["prediction"]["affected_resource"] == "eth3:vlan10"
        ...
        if deploy_attempted:
            ...
            if cleanup["return_code"] != 0 and primary_error is None:
                pytest.fail("X3-R3 Containerlab cleanup failed: " + str(cleanup["stderr"]))
        assert containerlab_containers(
