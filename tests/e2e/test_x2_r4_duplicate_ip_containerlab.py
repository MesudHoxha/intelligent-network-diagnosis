...
from src.orchestration.x2_duplicate_ip_experiment_runner import (
    run_x2_r4_experiment,
)
...
RUN_INFRASTRUCTURE = os.environ.get("IND_RUN_X2_R4_E2E") == "1"

pytestmark = [
    pytest.mark.infrastructure,
    pytest.mark.skipif(
        not RUN_INFRASTRUCTURE,
        reason="set IND_RUN_X2_R4_E2E=1 to run the X2-R4 Containerlab lifecycle",
    ),
]


def test_real_duplicate_ip_cycle_diagnoses_restores_and_cleans_up(
    tmp_path: Path,
) -> None:
    ...
    try:
        ...
        require_success(
            ...
            "X2-R4 Containerlab deployment",
        )
        configured_evidence_root = os.environ.get("IND_X2_R4_EVIDENCE_ROOT")
        output_root = (
        ...
        result = run_x2_r4_experiment(
            ...
            output_root,
            BASELINE,
            experiment_id="x2-r4-real-duplicate-ip",
        )
        ...
        assert diagnosis["prediction"]["fault_type"] == "duplicate_ip"
        ...
        if deploy_attempted:
            ...
            if cleanup["return_code"] != 0 and primary_error is None:
                pytest.fail("X2-R4 Containerlab cleanup failed: " + str(cleanup["stderr"]))
        assert containerlab_containers(
            repository_root=ROOT,
            command_executor=run_command,
