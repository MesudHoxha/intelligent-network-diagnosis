...
class X3VlanMissingError(Phase6FaultInjectionError):
    """Raised when the scoped X3-R2 runtime fails closed."""
...
def _require(condition: bool, message: str) -> None:
    if not condition:
        raise X3VlanMissingError(message)
...
def load_vlan_missing_scenario(path: Path) -> VlanMissingScenario:
    path = Path(path)
    try:
        document = yaml.safe_load(path.read_text(encoding="utf-8"))
    except (OSError, yaml.YAMLError) as error:
        raise X3VlanMissingError(f"Cannot read X3-R2 scenario: {path}") from error
    _require(isinstance(document, dict), "X3-R2 scenario must be an object.")
    _require(document.get("schema_version") == 1, "X3-R2 schema version drifted.")
    scenario = document.get("scenario")
    _require(isinstance(scenario, dict), "X3-R2 scenario object is missing.")
    assert isinstance(scenario, dict)
    _require(
        scenario.get("kind") == "fault"
        and scenario.get("truth_model") == "single_fault",
        "X3-R2 supports exactly one reviewed single fault.",
    )
    ...
    for value, label in (
        ...
        _require(isinstance(value, dict), f"X3-R2 {label} binding is missing.")
    ...
    _require(isinstance(parameters, dict), "X3-R2 mutation parameters are missing.")
    ...
    _require(
        fault.get("type") == "vlan_missing"
        and fault.get("injector")
        == "remove_expected_vlan_from_target_switch_memberships"
        and restoration.get("method")
        == "restore_exact_bridge_access_and_trunk_vlan_memberships",
        "X3-R2 fault or restoration mechanism drifted.",
    )
    _require(
        ...
        "X3-R2 mutation parameters drifted from observation context.",
    )
    _require(
        ground_truth.get("fault_category") == "l2_vlan"
        and ground_truth.get("fault_type") == "vlan_missing"
        and ground_truth.get("fault_location") == fault.get("target_node")
        and ground_truth.get("affected_resource")
        == f"{observation.get('bridge')}:vlan{expected_vlan}",
        "X3-R2 ground truth does not match the fault binding.",
    )
    ...
    try:
        ...
        raise X3VlanMissingError("X3-R2 contains invalid IPv4 data.") from error

    return VlanMissingScenario(
        path=path,
