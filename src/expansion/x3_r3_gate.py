...
from src.expansion.x3_vlan_not_allowed_on_trunk import load_vlan_not_allowed_on_trunk_scenario


MANIFEST_PATH = Path("plans/expansion/X3_R3_VLAN_NOT_ALLOWED_ON_TRUNK_V1.json")
SCHEMA_PATH = Path("schemas/x3_r3_vlan_not_allowed_on_trunk_gate_v1.schema.json")
EXPECTED_PARENT = "36c97479e7423be30e45b504aa82e65c42bf2101"
EXPECTED_RUNTIME = {
...
EXPECTED_SAFETY = (
    ...
    "EXACT_TRUNK_ALLOWED_VLAN_RESTORATION",
    "NATIVE_FLOW_PRESERVED",
)


class X3R3GateError(ValueError):
    """Raised when the X3-R3 runtime boundary drifts."""


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise X3R3GateError(message)
...
def _load(path: Path) -> dict[str, Any]:
    try:
        ...
        raise X3R3GateError(f"Cannot read X3-R3 JSON object: {path}") from error
    _require(isinstance(value, dict), f"X3-R3 JSON artifact is not an object: {path}")
    return value
...
def _safe(root: Path, relative: str) -> Path:
    ...
    _require(
        ...
        "X3-R3 source binding path is not canonical.",
    )
    path = root / relative
    resolved = path.resolve(strict=True)
    _require(
        resolved.is_relative_to(root.resolve()) and path.is_file() and not path.is_symlink(),
        f"X3-R3 source binding is unsafe: {relative}",
    )
    return path


def validate_x3_r3_manifest(
    ...
    if errors:
        ...
        raise X3R3GateError(
            f"X3-R3 schema validation failed at {path or '<root>'}: {errors[0].message}"
        )
    _require(
        manifest.get("runtime_authorization") == EXPECTED_RUNTIME,
        "X3-R3 runtime authorization drifted outside the scoped slice.",
    )
    ...
    _require(
        ...
        "X3-R3 parent or append-only boundary drifted.",
    )
    slice_definition = manifest.get("slice")
    _require(isinstance(slice_definition, Mapping), "X3-R3 slice is missing.")
    _require(
        slice_definition.get("fault_type") == "vlan_not_allowed_on_trunk"
        and slice_definition.get("rule_id") == "R_X3_L2_VLAN_003"
        and slice_definition.get("signature") == EXPECTED_SIGNATURE,
        "X3-R3 signature drifted.",
    )
    safety = manifest.get("safety")
    _require(
        isinstance(safety, Mapping)
        and tuple(safety.get("invariants", ())) == EXPECTED_SAFETY,
        "X3-R3 safety boundary drifted.",
    )
    ...
    _require(
        ...
        "X3-R3 acceptance or scientific boundary drifted.",
    )


def verify_x3_r3_gate(repository_root: Path) -> dict[str, Any]:
    root = Path(repository_root)
    manifest = _load(root / MANIFEST_PATH)
    schema = _load(root / SCHEMA_PATH)
    validate_x3_r3_manifest(manifest, schema)
    parent = verify_x3_r2_gate(root)
    _require(
        parent["status"] == "IMPLEMENTED_RUNTIME_SLICE",
        "X3-R2 parent is not verified.",
    )
    ...
    _require(
        ...
        "X3-R3 requires exactly 14 unique source bindings.",
    )
    for row in bindings:
        path = _safe(root, str(row["path"]))
        _require(
            hashlib.sha256(path.read_bytes()).hexdigest() == row["sha256"],
            f"X3-R3 source binding drifted: {row['path']}",
        )
    scenario = load_vlan_not_allowed_on_trunk_scenario(
        root / str(manifest["slice"]["scenario_path"])
    )
    _require(
        scenario.scenario_id == "X3_R3_VLAN_NOT_ALLOWED_ON_TRUNK"
        and scenario.topology_id == "X3_TOP_01_L2_VLAN"
        and (scenario.expected_vlan, scenario.native_vlan) == (10, 99)
        and scenario.affected_resource == "eth3:vlan10",
        "X3-R3 scenario identity drifted.",
    )
    ...
    _require(
        ...
        "X3-R3 topology observation roles drifted.",
    )
    return manifest


def main() -> int:
    parser = argparse.ArgumentParser(description="Verify the X3-R3 VLAN Not Allowed on Trunk gate.")
    parser.add_argument("--repository-root", type=Path, default=Path.cwd())
    arguments = parser.parse_args()
    manifest = verify_x3_r3_gate(arguments.repository_root)
    print("x3_r2_gate=VERIFIED")
    print("x3_r3_gate=VERIFIED")
    print("vlan_not_allowed_on_trunk_signature=TRUE_TRUE_FALSE_TRUE_TRUE_PASS")
