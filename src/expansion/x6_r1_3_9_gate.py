"""Append-only H/I successor gate with R1.3.8 in its published context."""
...
from src.orchestration.x6_r1_3_9_contract import ROOT, PREDECESSOR, RELEASE, HISTORICAL_VECTOR, digest, require
from src.runtime.subprocesses import run_capture

PLAN = "plans/expansion/X6_R1_3_9_CLEANUP_AND_TEARDOWN_OBSERVATION_CORRECTION_V1.json"
...
def verify_source(*, historical=True):
    ...
    if historical:
        with _detached_snapshot(ROOT, PREDECESSOR, receipts=[]) as snapshot:
            result = run_capture([sys.executable, "-c", "from src.expansion.x6_r1_3_8_gate import verify_source; verify_source()"], cwd=snapshot, timeout_seconds=1200)
            require(result.returncode == 0, "historical R1.3.8 gate failed in exact snapshot: " + result.stderr)
