"""Append-only R1.4 preparation gate with R1.3.9 in its published context."""
...
from src.orchestration.x6_r1_4_contract import ROOT, PREDECESSOR, RELEASE, HISTORICAL_VECTOR, digest, require
from src.runtime.subprocesses import run_capture

PLAN = "plans/expansion/X6_R1_4_BASELINE_ONLY_RUNTIME_AUTHORIZATION_PREPARATION_V1.json"
...
def verify_source(*, historical=True):
    ...
    require(plan["runtime_authorization_artifact"] == "ABSENT" and plan["authorization_template"] == "NON_EXECUTABLE", "source preparation cannot authorize runtime")
    ...
    if historical:
        with _detached_snapshot(ROOT, PREDECESSOR, receipts=[]) as snapshot:
            result = run_capture([sys.executable, "-c", "from src.expansion.x6_r1_3_9_gate import verify_source; verify_source()"], cwd=snapshot, timeout_seconds=1200)
            require(result.returncode == 0, "historical R1.3.9 gate failed in exact snapshot: " + result.stderr)
