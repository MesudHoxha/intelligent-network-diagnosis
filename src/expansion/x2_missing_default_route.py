...
class MissingDefaultRouteScenario:
    path: Path
    sha256: str
...
def load_missing_default_route_scenario(path: Path) -> MissingDefaultRouteScenario:
    path = Path(path)
    try:
        document = yaml.safe_load(path.read_text(encoding="utf-8"))
    except (OSError, yaml.YAMLError) as error:
        raise X2AddressingError(f"Cannot read X2-R3 scenario: {path}") from error
    _require(isinstance(document, dict), "X2-R3 scenario must be an object.")
    _require(document.get("schema_version") == 1, "X2-R3 schema version drifted.")
    scenario = document.get("scenario")
    _require(isinstance(scenario, dict), "X2-R3 scenario object is missing.")
    assert isinstance(scenario, dict)
    _require(
        scenario.get("kind") == "fault"
        and scenario.get("truth_model") == "single_fault",
        "X2-R3 supports exactly one reviewed single fault.",
    )
    ...
    for value, label in (
        ...
        _require(isinstance(value, dict), f"X2-R3 {label} binding is missing.")
    ...
    _require(
        fault.get("type") == "missing_default_route"
        and fault.get("injector") == "remove_exact_default_route"
        and restoration.get("method") == "restore_exact_default_route",
        "X2-R3 fault or restoration mechanism drifted.",
    )
    _require(
        ground_truth.get("fault_type") == "missing_default_route"
        and ground_truth.get("fault_category") == "addressing"
        and ground_truth.get("fault_location") == fault.get("target_node"),
        "X2-R3 ground truth does not match the fault binding.",
    )
    parameters = fault.get("parameters")
    _require(isinstance(parameters, dict), "X2-R3 parameters are missing.")
    ...
    try:
        ...
        raise X2AddressingError("X2-R3 contains invalid IPv4 data.") from error
    _require(
        expected_interface.network == expected_network and gateway in expected_network,
        "Missing-default-route slice requires a valid unchanged source identity and on-link gateway.",
    )
    _require(
        parameters.get("expected_gateway") == expected_gateway
        and parameters.get("source_interface") == observation.get("source_interface"),
        "X2-R3 mutation parameters drifted from observation context.",
    )

    return MissingDefaultRouteScenario(
        path=path,
