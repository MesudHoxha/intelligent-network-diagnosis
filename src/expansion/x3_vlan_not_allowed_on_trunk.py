...
class X3VlanNotAllowedOnTrunkError(Phase6FaultInjectionError):
    """Raised when the scoped X3-R3 runtime fails closed."""
...
class VlanNotAllowedOnTrunkScenario:
    path: Path
    sha256: str
...
def _require(condition: bool, message: str) -> None:
    if not condition:
        raise X3VlanNotAllowedOnTrunkError(message)
...
def load_vlan_not_allowed_on_trunk_scenario(path: Path) -> VlanNotAllowedOnTrunkScenario:
    path = Path(path)
    try:
        document = yaml.safe_load(path.read_text(encoding="utf-8"))
    except (OSError, yaml.YAMLError) as error:
        raise X3VlanNotAllowedOnTrunkError(f"Cannot read X3-R3 scenario: {path}") from error
    _require(isinstance(document, dict), "X3-R3 scenario must be an object.")
    _require(document.get("schema_version") == 1, "X3-R3 schema version drifted.")
    scenario = document.get("scenario")
    _require(isinstance(scenario, dict), "X3-R3 scenario object is missing.")
    assert isinstance(scenario, dict)
    _require(
        scenario.get("kind") == "fault"
        and scenario.get("truth_model") == "single_fault",
        "X3-R3 supports exactly one reviewed single fault.",
    )
    ...
    for value, label in (
        ...
        _require(isinstance(value, dict), f"X3-R3 {label} binding is missing.")
    ...
    _require(isinstance(parameters, dict), "X3-R3 mutation parameters are missing.")
    ...
    _require(
        expected_vlan != native_vlan,
        "Expected and native VLAN identities must be distinct.",
    )
    _require(
        fault.get("type") == "vlan_not_allowed_on_trunk"
        and fault.get("injector")
        == "remove_expected_tagged_vlan_from_one_trunk_endpoint"
        and restoration.get("method")
        == "restore_exact_trunk_allowed_vlan_set",
        "X3-R3 fault or restoration mechanism drifted.",
    )
    _require(
        ...
        and parameters.get("trunk_interface") == observation.get("trunk_interface")
        and parameters.get("expected_vlan") == expected_vlan,
        "X3-R3 mutation parameters drifted from observation context.",
    )
    _require(
        ground_truth.get("fault_category") == "l2_vlan"
        and ground_truth.get("fault_type") == "vlan_not_allowed_on_trunk"
        and ground_truth.get("fault_location") == fault.get("target_node")
        and ground_truth.get("affected_resource")
        == f"{observation.get('trunk_interface')}:vlan{expected_vlan}",
        "X3-R3 ground truth does not match the fault binding.",
    )
    ...
    try:
        ...
        raise X3VlanNotAllowedOnTrunkError("X3-R3 contains invalid IPv4 data.") from error

    return VlanNotAllowedOnTrunkScenario(
        path=path,
        ...
        affected_resource=_string(
            ground_truth.get("affected_resource"), "ground_truth.affected_resource"
        ),
    )
