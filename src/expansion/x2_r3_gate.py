...
from src.expansion.x2_missing_default_route import load_missing_default_route_scenario


MANIFEST_PATH = "plans/expansion/X2_R3_MISSING_DEFAULT_ROUTE_V1.json"
SCHEMA_PATH = "schemas/x2_r3_missing_default_route_gate_v1.schema.json"
EXPECTED_RUNTIME = {
...
class X2R3GateError(ValueError):
    """Raised when the X2-R3 release boundary drifts."""


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise X2R3GateError(message)
...
def _load(path: Path) -> dict[str, Any]:
    try:
        ...
        raise X2R3GateError(f"Cannot read X2-R3 JSON object: {path}") from error
    _require(isinstance(value, dict), f"X2-R3 JSON artifact is not an object: {path}")
    return value
...
def _safe(root: Path, relative: str) -> Path:
    ...
    try:
        ...
        raise X2R3GateError(f"X2-R3 source binding is unavailable: {relative}") from error
    _require(
        resolved.is_relative_to(root.resolve()) and path.is_file() and not path.is_symlink(),
        f"X2-R3 source binding is unsafe: {relative}",
    )
    return path


def validate_x2_r3_manifest(
    ...
    if errors:
        ...
        raise X2R3GateError(
            f"X2-R3 schema validation failed at {path or '<root>'}: "
            f"{errors[0].message}"
        )
    _require(
        manifest.get("runtime_authorization") == EXPECTED_RUNTIME,
        "X2-R3 runtime authorization drifted outside the scoped slice.",
    )
    compatibility = manifest.get("compatibility")
    _require(isinstance(compatibility, Mapping), "X2-R3 compatibility is missing.")
    _require(
        ...
        and compatibility.get("x2_r2_gate") == "UNCHANGED_AND_VERIFIED"
        and compatibility.get("extended_api") == "NOT_CREATED"
        and compatibility.get("p9_r1") == "PAUSED",
        "X2-R3 frozen compatibility boundary drifted.",
    )
    slice_definition = manifest.get("slice")
    _require(isinstance(slice_definition, Mapping), "X2-R3 slice is missing.")
    _require(
        slice_definition.get("fault_type") == "missing_default_route"
        and slice_definition.get("rule_id") == "R_X2_ADDRESSING_003"
        ...
        == ["wrong_ip_address", "wrong_subnet_mask", "duplicate_ip"],
        "X2-R3 signature, preserved rule, or confounder boundary drifted.",
    )
    collector = manifest.get("collector_activation")
    _require(isinstance(collector, Mapping), "X2-R3 collector activation is missing.")
    _require(
        ...
        "X2-R3 collector activation drifted.",
    )
    ...
    _require(
        ...
        "X2-R3 safety invariants drifted.",
    )
    acceptance = manifest.get("acceptance")
    _require(isinstance(acceptance, Mapping), "X2-R3 acceptance is missing.")
    _require(
        ...
        "X2-R3 acceptance or scientific boundary drifted.",
    )


def verify_x2_r3_gate(repository_root: Path) -> dict[str, Any]:
    root = Path(repository_root)
    manifest = _load(root / MANIFEST_PATH)
    schema = _load(root / SCHEMA_PATH)
    validate_x2_r3_manifest(manifest, schema)
    parent = verify_x2_r2_gate(root)
    _require(
        parent["status"] == "IMPLEMENTED_RUNTIME_SLICE"
        and parent["track"]["next_release"] == "X2_R3_MISSING_DEFAULT_ROUTE",
        "X2-R3 parent runtime gate is not accepted at the expected boundary.",
    )
    bindings = manifest["source_bindings"]
    assert isinstance(bindings, list)
    _require(
        len({row["binding_id"] for row in bindings}) == len(bindings),
        "X2-R3 source binding IDs must be unique.",
    )
    for row in bindings:
        path = _safe(root, str(row["path"]))
        _require(
            _sha256(path) == row["sha256"],
            f"X2-R3 source binding drifted: {row['path']}",
        )
    scenario_path = root / str(manifest["slice"]["scenario_path"])
    binding = load_missing_default_route_scenario(scenario_path)
    _require(
        binding.scenario_id == "X2_R3_MISSING_DEFAULT_ROUTE"
        and binding.expected_interface == "10.20.1.10/24"
        and binding.expected_gateway == "10.20.1.1",
        "X2-R3 scenario identity drifted.",
    )
    ...
    _require(
        ...
        "X2-R3 topology context does not match the scenario.",
    )
    return manifest
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Verify the X2-R3 Missing Default Route runtime gate."
    )
    parser.add_argument("--repository-root", type=Path, default=Path.cwd())
    arguments = parser.parse_args()
    manifest = verify_x2_r3_gate(arguments.repository_root)
    print("x2_r2_gate=VERIFIED_UNCHANGED")
    print("x2_r3_gate=VERIFIED")
    print("missing_default_route_signature=ADDRESS_TRUE_PREFIX_TRUE_DEFAULT_FALSE_DUPLICATE_FALSE_PASS")
    print("previous_addressing_rules=R_X2_ADDRESSING_001_AND_002_PRESERVED_PASS")
