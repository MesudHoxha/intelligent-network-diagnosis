...
def _load_object(path: Path) -> dict[str, object]:
    try:
        ...
        raise X3VlanMissingError(f"Cannot read X3-R2 JSON object: {path}") from error
    if not isinstance(value, dict):
        raise X3VlanMissingError(f"X3-R2 JSON artifact must be an object: {path}")
    return value
...
def collect_vlan_missing_evidence_v4(
    output_directory: Path,
    scenario_path: Path,
    *,
    ...
    if (output_directory / EVIDENCE_PATH).exists() or (
        ...
        raise X3VlanMissingError("X3-R2 Evidence v4 output already exists.")
    ...
    run = {
        ...
        "errors": [] if complete else ["One or more X3-R2 observations are unavailable."],
    }
    evidence = {
        "schema_version": 4,
    ...
    if context["context_id"] != binding.topology_context_id:
        raise X3VlanMissingError("X3-R2 topology context binding drifted.")
    ...
    if (
        ...
        raise X3VlanMissingError("X3-R2 observation roles drifted from Topology Context v1.")
    ...
    *,
    ...
    if not evidence_path.is_file():
        raise X3VlanMissingError(
            "X3-R2 Evidence v4 must be persisted before vectorization."
        )
    if vector_path.exists():
        raise X3VlanMissingError("X3-R2 Feature Vector v2 already exists.")
    observations = evidence.get("observations")
    if not isinstance(observations, Mapping):
        raise X3VlanMissingError("X3-R2 Evidence v4 has no observations.")
