...
RAW = Path("raw/v4/service_state_collector_v4")
...
def _obs(value: bool | None, availability: str, raw: tuple[str, str], collector_id: str = "service_state_collector_v4") -> dict[str, object]:
...
def _dns_query_state(result: Mapping[str, object], expected_answer: str) -> tuple[bool | None, bool | None, str]:
    ...
    if code == 0:
        if not answers: return False, False, "observed"
        return True, expected_answer in answers, "observed"
    if code in (1, 9): return False, False, "observed"
    return None, None, "collection_unavailable"


def collect_wrong_dns_record_evidence_v4(output_directory: Path, scenario_path: Path, *, executor: Phase6Executor = DEFAULT_EXECUTOR, repository_root: Path = ROOT) -> dict[str, object]:
    output = Path(output_directory)
    if (output / EVIDENCE).exists() or (output / RAW).exists(): raise X4WrongDnsRecordError("X4-R4 Evidence v4 output already exists.")
    binding = load_wrong_dns_record_scenario(scenario_path); started = utc_now()
    configuration = _result(executor, binding.destination_container, ["sh", "-eu", "-c", "cat /etc/x4-dns/dnsmasq.conf; dnsmasq --test --conf-file=/etc/x4-dns/dnsmasq.conf"]); configuration_raw = _raw(output, "dns_record_configuration", {"schema_version": 1, "probe_id": "direct_dns_record_configuration", "command_result": configuration, "controlled_wrong_record_line": binding.controlled_wrong_record_line})
    if configuration["return_code"] != 0 or binding.controlled_wrong_record_line not in str(configuration["stdout"]): raise X4WrongDnsRecordError("X4-R4 direct controlled DNS-record configuration evidence unavailable or inconsistent.")
    ...
    ping = _result(executor, binding.source_container, ["ping", "-c", "1", "-W", "1", binding.dns_server_address]); ping_raw = _raw(output, "dns_network_reachability", {"schema_version": 1, "probe_id": "dns_host_network_reachability_not_dns_answer", "command_result": ping})
    query = _result(executor, binding.source_container, ["sh", "-eu", "-c", "dig +norecurse +time=2 +tries=1 @" + binding.dns_server_address + " " + binding.expected_dns_name + " A +short"]); query_raw = _raw(output, "dns_direct_query_response", {"schema_version": 1, "probe_id": "direct_authoritative_dns_query_no_client_cache", "command_result": query, "expected_answer": binding.expected_dns_answer, "controlled_wrong_answer": binding.controlled_wrong_dns_answer})
    ...
    dhcp_lease, dhcp_scope, dhcp_availability = _dhcp_state(dhcp, binding.expected_scope_prefix); ping_value, ping_availability = _binary(ping); query_value, answer_value, query_availability = _dns_query_state(query, binding.expected_dns_answer); process_value, process_availability = _binary(process); port_value, port_availability = _binary(port); app_process_value, app_process_availability = _binary(app_process); app_port_value, app_port_availability = _binary(app_port); policy_value, policy_availability = _binary(policy)
    observations = {"dhcp_server_reachable": _obs(dhcp_lease, dhcp_availability, dhcp_raw), "dhcp_lease_obtained": _obs(dhcp_lease, dhcp_availability, dhcp_raw), "dhcp_lease_matches_expected_scope": _obs(dhcp_scope, dhcp_availability, dhcp_raw), "dns_server_reachable": _obs(ping_value, ping_availability, ping_raw), "dns_query_succeeds": _obs(query_value, query_availability, query_raw), "dns_answer_matches_expected": _obs(answer_value, query_availability, query_raw), "service_process_running": _obs(process_value, process_availability, process_raw), "service_port_reachable": _obs(port_value, port_availability, port_raw), "service_flow_blocked_by_policy": _obs(None if policy_availability != "observed" else "X4-R1-SERVICE-BLOCK" in str(policy["stdout"]), policy_availability, policy_raw, "service_policy_state_collector")}
    service_availability = (dhcp_availability, ping_availability, query_availability, process_availability, port_availability, app_process_availability, app_port_availability); raw_artifacts = [{"path": p, "sha256": h} for p, h in (configuration_raw, dhcp_raw, ping_raw, query_raw, process_raw, port_raw, app_process_raw, app_port_raw)]; completed = utc_now(); errors = [] if all(value == "observed" for value in service_availability) else ["A DNS/DHCP/service collection command was unavailable."]
    evidence = {"schema_version": 4, "evidence_id": binding.scenario_id.lower() + ":evidence:v4", "topology_context_id": binding.topology_context_id, "collected_at_utc": completed, "observation_path": {"direction": "client_to_dns_server", "source_node": binding.source_node, "destination_node": binding.destination_node, "observer_nodes": ["observer"]}, "collector_runs": [{"schema_version": 1, "collector_id": "service_state_collector_v4", "collector_version": 4, "domain": "services", "status": "completed" if not errors else "partial", "started_at_utc": started, "completed_at_utc": completed, "feature_ids": list(FEATURE_IDS[:-1]), "raw_artifacts": raw_artifacts, "errors": errors}, {"schema_version": 1, "collector_id": "service_policy_state_collector", "collector_version": 1, "domain": "security", "status": "completed" if policy_availability == "observed" else "partial", "started_at_utc": started, "completed_at_utc": completed, "feature_ids": ["service_flow_blocked_by_policy"], "raw_artifacts": [{"path": policy_raw[0], "sha256": policy_raw[1]}], "errors": [] if policy_availability == "observed" else ["Policy collection unavailable."]}], "observations": observations, "compatibility": {"origin": "native_v4", "source_schema_version": None, "source_artifact_sha256": None}}
    ...
    if context["context_id"] != binding.topology_context_id or context["observation_roles"] != {"source": "client", "destination": "dns_server", "observers": ["observer"]}: raise X4WrongDnsRecordError("X4-R4 must reuse the accepted client-to-DNS-server context.")
    validate_evidence_v4(evidence, catalog, repository_root=repository_root); write_json_atomic(output / EVIDENCE, evidence)
    return evidence


def build_service_feature_vector_v2_r4(output_directory: Path, evidence: Mapping[str, object], *, repository_root: Path = ROOT) -> dict[str, object]:
    output = Path(output_directory); evidence_path = output / EVIDENCE
    if not evidence_path.is_file() or (output / VECTOR).exists(): raise X4WrongDnsRecordError("X4-R4 Evidence v4 persistence/vector output boundary failed.")
    ...
    if not isinstance(observations, Mapping): raise X4WrongDnsRecordError("X4-R4 evidence observations missing.")
