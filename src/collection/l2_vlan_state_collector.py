...
def _load_object(path: Path) -> dict[str, object]:
    try:
        ...
        raise X3WrongAccessVlanError(f"Cannot read X3-R1 JSON object: {path}") from error
    if not isinstance(value, dict):
        raise X3WrongAccessVlanError(f"X3-R1 JSON artifact must be an object: {path}")
    return value
...
def collect_wrong_access_vlan_evidence_v4(
    output_directory: Path,
    scenario_path: Path,
    *,
    ...
    if (output_directory / EVIDENCE_PATH).exists() or (
        ...
        raise X3WrongAccessVlanError("X3-R1 Evidence v4 output already exists.")
    ...
    wrong_access = vlan_membership(
        sw1_vlans, binding.target_access_interface, binding.wrong_vlan
    )
    ...
    access_value = (
        is_pvid_untagged(expected_access) and wrong_access is None
        if vlan_complete
        else None
    )
    ...
    run = {
        ...
        "errors": [] if complete else ["One or more X3-R1 observations are unavailable."],
    }
    evidence = {
        "schema_version": 4,
    ...
    if context["context_id"] != binding.topology_context_id:
        raise X3WrongAccessVlanError("X3-R1 topology context binding drifted.")
    ...
    if (
        ...
        raise X3WrongAccessVlanError("X3-R1 observation roles drifted from Topology Context v1.")
    ...
    *,
    ...
    if not evidence_path.is_file():
        raise X3WrongAccessVlanError(
            "X3-R1 Evidence v4 must be persisted before vectorization."
        )
    if vector_path.exists():
        raise X3WrongAccessVlanError("X3-R1 Feature Vector v2 already exists.")
    observations = evidence.get("observations")
    if not isinstance(observations, Mapping):
        raise X3WrongAccessVlanError("X3-R1 Evidence v4 has no observations.")
