...
def _load_object(path: Path) -> dict[str, object]:
    try:
        ...
        raise X2AddressingError(f"Cannot read X2-R2 JSON object: {path}") from error
    if not isinstance(value, dict):
        raise X2AddressingError(f"X2-R2 JSON artifact must be an object: {path}")
    return value
...
def collect_wrong_subnet_mask_evidence_v4(
    output_directory: Path,
    scenario_path: Path,
    *,
    ...
    if evidence_path.exists() or (output_directory / RAW_DIRECTORY).exists():
        raise X2AddressingError("X2-R2 Evidence v4 output already exists.")
    ...
    errors = (
        ...
        else ["One or more X2-R2 required addressing observations are unavailable."]
    )
    completed = utc_now()
    collector_run = {
        "schema_version": 1,
    ...
    if context["context_id"] != binding.topology_context_id:
        raise X2AddressingError("X2-R2 topology context binding drifted.")
    ...
    for artifact in collector_run["raw_artifacts"]:
        ...
        if hashlib.sha256(path.read_bytes()).hexdigest() != artifact["sha256"]:
            raise X2AddressingError("X2-R2 raw evidence hash verification failed.")
    return evidence
