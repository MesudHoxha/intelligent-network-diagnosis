...
def _identity_matches(record: dict[str, Any], binding: WrongAccessVlanScenario) -> bool:
    return all(record.get(name) == value for name, value in binding.recovery_identity.items())


def _require_identity(
    record: dict[str, Any],
    binding: WrongAccessVlanScenario,
    label: str,
) -> None:
    if not _identity_matches(record, binding):
        raise X3WrongAccessVlanError(
            f"{label} does not match the reviewed X3-R1 scenario."
        )
...
def _ensure_new_output(output_directory: Path) -> None:
    ...
    if existing:
        raise X3WrongAccessVlanError(
            "X3-R1 mutation output already exists: " + ", ".join(existing)
        )
...
def _access_checks(
    ...
    *,
    ...
    target_result, target_rows = bridge_vlan_inventory(
        ...
        binding.target_access_interface,
    )
    expected = vlan_membership(
        target_rows, binding.target_access_interface, binding.expected_vlan
    )
    wrong = vlan_membership(
        target_rows, binding.target_access_interface, binding.wrong_vlan
    )
    if faulted:
        return {
            "wrong_access_vlan_is_exact_pvid_untagged": _check(
                is_pvid_untagged(wrong), target_result, target_rows
            ),
            "expected_access_vlan_is_absent": _check(
                expected is None, target_result, target_rows
            ),
        }
    return {
        "expected_access_vlan_is_exact_pvid_untagged": _check(
            is_pvid_untagged(expected), target_result, target_rows
        ),
        "controlled_wrong_access_vlan_is_absent": _check(
            wrong is None, target_result, target_rows
        ),
    }
...
def _baseline_checks(
    ...
    checks = _access_checks(binding, executor, faulted=False)
...
def _fault_checks(
    ...
    checks = _access_checks(binding, executor, faulted=True)
    tagged_result, tagged_reachable = ping_result(
        executor, binding.source_container, binding.destination_address
    )
    native_result, native_reachable = ping_result(
...
def _require_recovery_authority(
    ...
    if not intent_path.is_file():
        raise X3WrongAccessVlanError(
            "X3-R1 restoration requires a matching durable recovery intent."
        )
    ...
    if intent.get("status") != "RECOVERY_REQUIRED_IF_MUTATION_ATTEMPTED":
        raise X3WrongAccessVlanError("X3-R1 recovery intent has an invalid state.")


def restore_wrong_access_vlan(
    scenario_path: Path,
    output_directory: Path,
    *,
    ...
    command = execute_checked(
        ...
        [
            ...
            (
                ...
                f"vid {binding.wrong_vlan} 2>/dev/null || true; "
                f"bridge vlan del dev {binding.target_access_interface} "
    ...
    if not confirmed:
        raise X3WrongAccessVlanError(
            "X3-R1 exact access-VLAN restoration was not confirmed."
        )
    return record


def inject_wrong_access_vlan(
    scenario_path: Path,
    output_directory: Path,
    *,
    executor: Phase6Executor = DEFAULT_EXECUTOR,
    ...
    if not _all_pass(preconditions):
        raise X3WrongAccessVlanError("X3-R1 baseline failed; no mutation was attempted.")
    ...
    try:
        command = execute_checked(
            ...
            [
                ...
                (
                    ...
                    f"bridge vlan add dev {binding.target_access_interface} "
                    f"vid {binding.wrong_vlan} pvid untagged"
                ),
            ],
        )
        postconditions = (
        ...
        if not confirmed:
            raise X3WrongAccessVlanError("X3-R1 fault postconditions were not confirmed.")
        return record
    except BaseException as primary:
        try:
            restore_wrong_access_vlan(scenario_path, output_directory, executor=executor)
        except BaseException as restoration:
            raise X3WrongAccessVlanError(
                "X3-R1 injection and restoration both failed."
            ) from restoration
        raise primary
