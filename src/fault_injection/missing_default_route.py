...
def _destination_matches_expected(
    command_result: dict[str, object],
    reachable: bool | None,
    expected: bool,
) -> bool:
    if reachable is expected:
        return True
    if expected or reachable is not None:
        return False
    stderr = str(command_result.get("stderr", "")).lower()
    return command_result.get("return_code") == 2 and (
        "network is unreachable" in stderr or "no route to host" in stderr
    )


def _identity_matches(
    record: dict[str, Any], binding: MissingDefaultRouteScenario
) -> bool:
...
def _require_identity(
    ...
    if not _identity_matches(record, binding):
        raise X2AddressingError(f"{label} does not match the reviewed X2-R3 scenario.")
...
def _ensure_new_output(output_directory: Path) -> None:
    ...
    if existing:
        raise X2AddressingError(
            "X2-R3 mutation output already exists: " + ", ".join(existing)
        )
...
def _state_checks(
    ...
    *,
    route_present: bool,
    destination_reachable_expected: bool,
    ...
    return {
        ...
        "destination_reachable": _check(
            _destination_matches_expected(
                destination_result,
                destination_reachable,
                destination_reachable_expected,
            ),
            destination_result,
            destination_reachable,
        ),
...
def _require_recovery_authority(
    ...
    if intent_path.exists():
        ...
        if intent.get("status") != "RECOVERY_REQUIRED_IF_MUTATION_ATTEMPTED":
            raise X2AddressingError("X2-R3 recovery intent has an invalid state.")
        return
    if injection_path.exists():
        ...
        if record.get("mutation_applied") is True:
            return
    raise X2AddressingError(
        "X2-R3 restoration requires a matching durable recovery intent."
    )


def restore_missing_default_route(
    scenario_path: Path,
    output_directory: Path,
    *,
    ...
    postconditions = (
        _state_checks(
            ...
            executor,
            route_present=True,
            destination_reachable_expected=True,
        )
    ...
    if not confirmed:
        raise X2AddressingError(
            "X2-R3 exact default-route restoration was not confirmed."
        )
    return record


def inject_missing_default_route(
    scenario_path: Path,
    output_directory: Path,
    *,
    executor: Phase6Executor = DEFAULT_EXECUTOR,
    ...
    if not _all_pass(preconditions):
        ...
        raise X2AddressingError(
            "X2-R3 default-route preconditions failed; no mutation was attempted."
        )
    ...
    try:
        mutation_command = execute_checked(
            ...
            [
                ...
                "-c",
                (
                    f"ip route del default via {binding.expected_gateway} "
                    f"dev {binding.source_interface}"
                ),
            ],
        )
        postconditions = (
            _state_checks(
                ...
                executor,
                route_present=False,
                destination_reachable_expected=False,
            )
        ...
        if not confirmed:
            raise X2AddressingError("X2-R3 missing default route was not confirmed.")
        return record
        ...
        try:
            ...
            raise X2AddressingError(
                "X2-R3 injection failed and best-effort restoration also failed."
