...
from src.orchestration.x6_r1_3_9_contract import ROOT, RELEASE, WINDOW_IDS, Invalid, canonical, digest, identity, load, require, validate_authorization
from src.orchestration.x6_r1_3_9_durable import Recorder, alive, fsync_directory, process_identity, records, reservation_path, reserve, save, stamp, acquire_topology_lock
from src.orchestration.x6_r1_3_9_observations import catalog, cleanup_cycle, controls, host_state, reconstruct_window, server_process_state, success, bootstrap, BOOTSTRAP_NAMES
...
def cleanup(recorder, before, *, require_complete=False):
    ...
    if current["containers"]:
        ...
        from src.orchestration.x6_r1_3_9_observations import validate_owned_cleanup_state
...
def assert_external_simulation():
    import shutil
    stub = ROOT / "tests/fixtures/x6_r1_3_9_external_stub.py"
...
def main():
    parser = argparse.ArgumentParser(description="R1.3.9 separately authorized baseline-only successor")
