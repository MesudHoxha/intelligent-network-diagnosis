"""R1.4.1 authorization commitment and historical-validation contract."""
...
from src.orchestration.x6_r1_4_contract import (
    BOOTSTRAP,
    CONTEXT,
    HISTORICAL_VECTOR,
    IMAGE,
...
PREDECESSOR = "9e2dee01d3f486cd0a523944523a1885bf8b9fac"
RELEASE = "X6_R1_4_1_AUTHORIZATION_COMMITMENT_AND_BOOT_RECOVERY_CORRECTION"
AUTHORIZATION_RELEASE = "X6_R1_4_1_BASELINE_ONLY_RUNTIME_AUTHORIZATION"
...
def identity(root=ROOT):
    ...
    paths = sorted(set(git("ls-files").splitlines()) | {
        ...
        for pattern in ("src/orchestration/x6_r1_4_1*.py", "src/expansion/x6_r1_4_1*.py", "plans/expansion/X6_R1_4_1*.json", "tests/fixtures/x6_r1_4_1_external_stub.py")
...
def frozen_contract():
    ...
    return {
        ...
        "maximum_attempts": 1,
        "historical_authorization": copy.deepcopy(HISTORICAL_VECTOR),
        "schedule": copy.deepcopy(SCHEDULE),
        "traffic": context["traffic"],
        "topology": TOPOLOGY,
        "image": IMAGE,
        "prohibitions": copy.deepcopy(PROHIBITIONS),
        "construction": list(WINDOW_IDS[:10]),
...
def validate_authorization_integrity(value, *, root, run_id, source_identity, simulation=False):
    ...
    require(value["schema_version"] == 4 and value["record_kind"] == "CONCRETE_AUTHORIZATION" and value["release_id"] == AUTHORIZATION_RELEASE, "authorization release")
