...
from src.collection.x6_performance_collector import FEATURES, aggregate_windows, exact_fault_hierarchy, exact_noqueue, qdisc_dropped, validate_speed_pair
from src.expansion.x6_r1_5_evidence import materialize_f1_evidence
...
from src.orchestration.x6_r1_5_contract import ACCEPTED_IMAGE, MANIFEST_EMBEDDED_SHA256, PHASE_BY_WINDOW, ROOT, RELEASE, WINDOW_IDS, accepted_manifest, canonical, digest, identity, load, require, validate_new_attempt
from src.orchestration.x6_r1_5_durable import Recorder, acquire_topology_lock, advance_clock, records, reserve, save, stamp
from src.orchestration.x6_r1_5_observations import BOOTSTRAP_NAMES, bootstrap, catalog, cleanup_cycle, controls, host_state, reconstruct_window, server_process_state, success
...
def phase_wait(root, name, seconds):
    start = stamp()
    if os.environ.get("X6_R1_5_CONTROLLED_CLOCK") == "1":
...
def cleanup(recorder, before, *, require_complete=False):
    ...
    if current["containers"]:
        from src.orchestration.x6_r1_5_observations import validate_owned_cleanup_state
...
def fault_outputs(recorder, fault_windows, manifest):
    wrapped = [{name: {"availability": "observed", "value": row["measurements"][name]} for name in FEATURES} for row in fault_windows]
    values = aggregate_windows(wrapped)
    all_rows = records(recorder.root, catalog())
    replies = sorted(float(rtt) for row in all_rows if row.get("window") in WINDOW_IDS[10:13] and row["name"] == "traffic_ping" for _, rtt in _REPLY.findall(row["stdout"]))
    require(replies, "fault RTT samples absent")
    values["round_trip_latency_ms_p95"] = {"availability": "observed", "value": replies[math.ceil(.95 * len(replies)) - 1]}
    ...
    evidence, vector = materialize_f1_evidence(
    ...
    require(predicates is not None and all(predicates.values()) and diagnosis["status"] == "diagnosed", "exact F1 signature absent")
...
def lifecycle(recorder, source):
    ...
    fault_outputs(recorder, fault, manifest)
    restore(recorder)
...
def execute(auth_path, root, run_id, *, simulation=False):
    require(simulation or os.environ.get("X6_R1_5_CONTROLLED_CLOCK") != "1", "controlled clock cannot enter production")
