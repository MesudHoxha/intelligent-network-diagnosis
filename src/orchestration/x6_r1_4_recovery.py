...
from src.orchestration.x6_r1_4_contract import RELEASE, canonical_location, identity, load, require, validate_authorization
from src.orchestration.x6_r1_4_durable import Recorder, alive, process_identity, records, reservation_path, save, stamp, acquire_topology_lock
from src.orchestration.x6_r1_4_observations import catalog, failed_command_orders
from src.orchestration.x6_r1_4_production_path import cleanup, terminal, assert_external_simulation


def recover(root, *, simulation=False):
    if simulation: assert_external_simulation()
    root = canonical_location(str(root))
    candidates = [load(p) for p in (root.parent / ".x6-r1-4-consumption").glob("*.json") if not p.name.endswith(".interrupted.json")]
...
def main():
    parser = argparse.ArgumentParser(description="R1.4 standalone owned-resource recovery/replay")
