...
class X2AddressingExperimentError(RuntimeError):
    """Raised when the isolated X2-R1 lifecycle cannot be accepted."""
...
def recover_x2_r1_experiment(
    ...
    *,
    ...
    if not experiment_directory.is_dir() or not _recovery_exists(mutation_directory):
        raise X2AddressingExperimentError(
            "X2-R1 recovery requires an existing matching recovery journal."
        )
    ...
    if restoration.get("status") != "RESTORATION_CONFIRMED":
        raise X2AddressingExperimentError("X2-R1 recovery was not confirmed.")
    ...
    if baseline.get("return_code") != 0:
        raise X2AddressingExperimentError(
            "X2-R1 restoration completed but baseline recovery failed."
        )
    result = {
        "schema_version": 1,
        "release_id": "X2_R1_WRONG_IP_ADDRESS",
...
def run_x2_r1_experiment(
    scenario_path: Path,
    output_root: Path,
    baseline_validator_path: Path,
    *,
    baseline_validator: BaselineValidator = run_baseline_validator,
    fault_injector: FaultMutator = inject_wrong_ip_address,
    fault_restorer: FaultMutator = restore_wrong_ip_address,
    evidence_collector: EvidenceCollector = collect_wrong_ip_evidence_v4,
    vector_builder: VectorBuilder = build_feature_vector_v2,
    rule_engine: RuleEngine = diagnose_wrong_ip_v2,
    ...
    try:
        ...
        if not baseline_before_passed:
            raise X2AddressingExperimentError(
                "X2-R1 baseline validation failed before mutation."
            )
        ...
        if (
            ...
            or diagnosis["prediction"].get("fault_type") != "wrong_ip_address"
        ):
            raise X2AddressingExperimentError(
                "X2-R1 exact Wrong IP rule did not produce the expected diagnosis."
            )
    ...
    if _recovery_exists(mutation_directory):
        try:
            ...
            if restoration.get("status") != "RESTORATION_CONFIRMED":
                raise X2AddressingExperimentError(
                    "X2-R1 restoration record is not confirmed."
                )
        except BaseException as error:
    ...
    if baseline_before_passed and restoration_error is None:
        ...
        if baseline_after.get("return_code") != 0:
            restoration_error = X2AddressingExperimentError(
                "X2-R1 baseline validation failed after restoration."
            )
    ...
    if final_error is not None:
        ...
        write_json_atomic(experiment_directory / "manifest.json", manifest)
        raise X2AddressingExperimentError(
            f"X2-R1 experiment failed: {final_error}. Artifacts: "
            f"{experiment_directory}"
        ) from final_error

    if not _restoration_confirmed(mutation_directory):
        raise X2AddressingExperimentError(
            "X2-R1 completed without a confirmed restoration record."
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the isolated X2-R1 Wrong IP Address lifecycle."
    )
    parser.add_argument("--scenario", type=Path, required=True)
    parser.add_argument("--output-root", type=Path, default=Path("data/raw/x2_r1"))
...
def main() -> int:
    ...
    try:
        if arguments.recover_experiment_directory is not None:
            result = recover_x2_r1_experiment(
                arguments.scenario,
    ...
    except (X2AddressingError, X2AddressingExperimentError, OSError) as error:
        print(f"[ERROR] {error}")
        return 1
