...
class X2SubnetMaskExperimentError(RuntimeError):
    """Raised when the isolated X2-R2 lifecycle cannot be accepted."""
...
def recover_x2_r2_experiment(
    ...
    *,
    ...
    if not experiment_directory.is_dir() or not _recovery_exists(mutation_directory):
        raise X2SubnetMaskExperimentError(
            "X2-R2 recovery requires an existing matching recovery journal."
        )
    restoration = fault_restorer(scenario_path, mutation_directory)
    if restoration.get("status") != "RESTORATION_CONFIRMED":
        raise X2SubnetMaskExperimentError("X2-R2 recovery was not confirmed.")
    ...
    if baseline.get("return_code") != 0:
        raise X2SubnetMaskExperimentError(
            "X2-R2 restoration completed but baseline recovery failed."
        )
    result = {
        "schema_version": 1,
        "release_id": "X2_R2_WRONG_SUBNET_MASK",
...
def run_x2_r2_experiment(
    scenario_path: Path,
    output_root: Path,
    baseline_validator_path: Path,
    *,
    baseline_validator: BaselineValidator = run_baseline_validator,
    fault_injector: FaultMutator = inject_wrong_subnet_mask,
    fault_restorer: FaultMutator = restore_wrong_subnet_mask,
    evidence_collector: EvidenceCollector = collect_wrong_subnet_mask_evidence_v4,
    ...
    try:
        ...
        if not baseline_before_passed:
            raise X2SubnetMaskExperimentError(
                "X2-R2 baseline validation failed before mutation."
            )
        ...
        if (
            ...
            or diagnosis["prediction"].get("fault_type") != "wrong_subnet_mask"
        ):
            raise X2SubnetMaskExperimentError(
                "X2-R2 exact Wrong Subnet Mask rule did not produce the expected diagnosis."
            )
    ...
    if _recovery_exists(mutation_directory):
        try:
            ...
            if restoration.get("status") != "RESTORATION_CONFIRMED":
                raise X2SubnetMaskExperimentError(
                    "X2-R2 restoration record is not confirmed."
                )
        except BaseException as error:
    ...
    if baseline_before_passed and restoration_error is None:
        ...
        if baseline_after.get("return_code") != 0:
            restoration_error = X2SubnetMaskExperimentError(
                "X2-R2 baseline validation failed after restoration."
            )
    ...
    if final_error is not None:
        ...
        write_json_atomic(experiment_directory / "manifest.json", manifest)
        raise X2SubnetMaskExperimentError(
            f"X2-R2 experiment failed: {final_error}. Artifacts: "
            f"{experiment_directory}"
        ) from final_error

    if not _restoration_confirmed(mutation_directory):
        raise X2SubnetMaskExperimentError(
            "X2-R2 completed without a confirmed restoration record."
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the isolated X2-R2 Wrong Subnet Mask lifecycle."
    )
    parser.add_argument("--scenario", type=Path, required=True)
    parser.add_argument("--output-root", type=Path, default=Path("data/raw/x2_r2"))
...
def main() -> int:
    ...
    try:
        if arguments.recover_experiment_directory is not None:
            result = recover_x2_r2_experiment(
                arguments.scenario,
    ...
    except (X2AddressingError, X2SubnetMaskExperimentError, OSError) as error:
        print(f"[ERROR] {error}")
        return 1
