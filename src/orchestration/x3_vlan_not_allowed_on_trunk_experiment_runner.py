...
class X3VlanNotAllowedOnTrunkExperimentError(RuntimeError):
    """Raised when the X3-R3 lifecycle cannot complete safely."""
...
def recover_x3_r3_experiment(
    ...
    *,
    ...
    if not experiment_directory.is_dir() or not _recovery_exists(mutation_directory):
        raise X3VlanNotAllowedOnTrunkExperimentError(
            "X3-R3 recovery requires an existing matching recovery journal."
        )
    restoration = fault_restorer(scenario_path, mutation_directory)
    if restoration.get("status") != "RESTORATION_CONFIRMED":
        raise X3VlanNotAllowedOnTrunkExperimentError("X3-R3 recovery was not confirmed.")
    ...
    if baseline.get("return_code") != 0:
        raise X3VlanNotAllowedOnTrunkExperimentError(
            "X3-R3 restoration completed but baseline recovery failed."
        )
    result = {
        "schema_version": 1,
        "release_id": "X3_R3_VLAN_NOT_ALLOWED_ON_TRUNK",
...
def run_x3_r3_experiment(
    scenario_path: Path,
    output_root: Path,
    baseline_validator_path: Path,
    *,
    baseline_validator: BaselineValidator = run_baseline_validator,
    fault_injector: FaultMutator = inject_vlan_not_allowed_on_trunk,
    fault_restorer: FaultMutator = restore_vlan_not_allowed_on_trunk,
    evidence_collector: EvidenceCollector = collect_vlan_not_allowed_on_trunk_evidence_v4,
    vector_builder: VectorBuilder = build_l2_vlan_feature_vector_v2,
    rule_engine: RuleEngine = diagnose_l2_vlan_x3_r3_v2,
    ...
    try:
        ...
        if not baseline_before_passed:
            raise X3VlanNotAllowedOnTrunkExperimentError(
                "X3-R3 baseline validation failed before mutation."
            )
        ...
        if (
            ...
            or diagnosis["prediction"].get("fault_type") != "vlan_not_allowed_on_trunk"
        ):
            raise X3VlanNotAllowedOnTrunkExperimentError(
                "X3-R3 exact VLAN Not Allowed on Trunk rule did not produce the expected diagnosis."
            )
    ...
    if _recovery_exists(mutation_directory):
        try:
            ...
            if restoration.get("status") != "RESTORATION_CONFIRMED":
                raise X3VlanNotAllowedOnTrunkExperimentError(
                    "X3-R3 restoration record is not confirmed."
                )
        except BaseException as error:
    ...
    if baseline_before_passed and restoration_error is None:
        ...
        if baseline_after.get("return_code") != 0:
            restoration_error = X3VlanNotAllowedOnTrunkExperimentError(
                "X3-R3 baseline validation failed after restoration."
            )
    ...
    if final_error is not None:
        ...
        write_json_atomic(experiment_directory / "manifest.json", manifest)
        raise X3VlanNotAllowedOnTrunkExperimentError(
            f"X3-R3 experiment failed: {final_error}. Artifacts: {experiment_directory}"
        ) from final_error

    if not _restoration_confirmed(mutation_directory):
        raise X3VlanNotAllowedOnTrunkExperimentError(
            "X3-R3 completed without a confirmed restoration record."
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the isolated X3-R3 VLAN Not Allowed on Trunk lifecycle."
    )
    parser.add_argument("--scenario", type=Path, required=True)
    parser.add_argument("--output-root", type=Path, default=Path("data/raw/x3_r3"))
...
def main() -> int:
    ...
    try:
        if arguments.recover_experiment_directory is not None:
            result = recover_x3_r3_experiment(
                arguments.scenario,
    ...
    except (X3VlanNotAllowedOnTrunkError, X3VlanNotAllowedOnTrunkExperimentError, OSError) as error:
        print(f"[ERROR] {error}")
        return 1
