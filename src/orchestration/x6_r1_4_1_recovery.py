"""Standalone R1.4.1 classification, owned cleanup, and read-only replay."""
from __future__ import annotations
import argparse
import fcntl
import os
from pathlib import Path
from src.orchestration.x6_r1_4_1_contract import RELEASE, canonical_location, identity, load, require, validate_committed_attempt, validate_partial_attempt
from src.orchestration.x6_r1_4_1_durable import Recorder, acquire_topology_lock, alive, process_identity, records, reservation_path, save, stamp
from src.orchestration.x6_r1_4_1_observations import catalog, failed_command_orders
from src.orchestration.x6_r1_4_1_production_path import assert_external_simulation, cleanup, terminal
...
def recover(root, *, simulation=False):
    ...
    try:
        ...
        if journal["sessions"] and journal["sessions"][-1]["status"] == "STARTED":
            ...
            require(all(row["intent_at"]["boot_id"] == interrupted["boot_id"] and row["phase"] in {"cleanup_before", "cleanup", "cleanup_after"} for row in selected), "interrupted recovery command scope/boot")
        ...
        try:
            ...
            raise
        if old and old["status"] in {"COLLECTION_COMPLETE_REPLAY_REQUIRED", "SOURCE_CONTRACT_COMPLETE_REPLAY_VERIFIED"}:
...
def main():
    parser = argparse.ArgumentParser(description="R1.4.1 standalone classification, owned cleanup, and read-only replay")
