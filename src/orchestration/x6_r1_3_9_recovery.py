...
from src.orchestration.x6_r1_3_9_contract import RELEASE, canonical_location, identity, load, require, validate_authorization
from src.orchestration.x6_r1_3_9_durable import Recorder, alive, process_identity, records, reservation_path, save, stamp, acquire_topology_lock
from src.orchestration.x6_r1_3_9_observations import catalog, failed_command_orders
from src.orchestration.x6_r1_3_9_production_path import cleanup, terminal, assert_external_simulation


def recover(root, *, simulation=False):
    if simulation: assert_external_simulation()
    root = canonical_location(str(root))
    candidates = [load(p) for p in (root.parent / ".x6-r1-3-9-consumption").glob("*.json") if not p.name.endswith(".interrupted.json")]
    ...
    try:
        ...
        save(root / "state/recovery.json", {"release_id": RELEASE, "original_process": ledger["process"], "recovery_process": process_identity(), "at": stamp(), "interrupted_orders": [row["order"] for row in rows if row.get("incomplete")], "nonzero_orders": [row["order"] for row in rows if row.get("return_code", 0) != 0], "failed_orders": failed_command_orders(rows), "source_test_only": simulation})
...
def main():
    parser = argparse.ArgumentParser(description="R1.3.9 standalone owned-resource recovery/replay")
