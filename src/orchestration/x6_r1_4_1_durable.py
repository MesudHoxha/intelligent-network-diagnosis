"""Fsynced one-attempt commitment and boot-bound observations for R1.4.1."""
...
from src.orchestration.x6_r1_4_1_contract import canonical, canonical_location, current_boot_id, digest, load, require
...
def stamp():
    return {"monotonic_ns": time.monotonic_ns(), "utc": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"), "boot_id": current_boot_id()}
...
def reservation_path(root, auth):
    return canonical_location(str(root)).parent / ".x6-r1-4-1-consumption" / (auth["authorization_id"] + ".json")
