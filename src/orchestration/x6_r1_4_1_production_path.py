...
from src.orchestration.x6_r1_4_1_contract import ROOT, RELEASE, WINDOW_IDS, Invalid, canonical, digest, identity, load, require, validate_new_attempt
from src.orchestration.x6_r1_4_1_durable import Recorder, alive, fsync_directory, process_identity, records, reservation_path, reserve, save, stamp, acquire_topology_lock
from src.orchestration.x6_r1_4_1_observations import catalog, cleanup_cycle, controls, host_state, reconstruct_window, server_process_state, success, bootstrap, BOOTSTRAP_NAMES
...
def teardown(recorder, rows, window_id):
    ...
    if server_process_state(*probes) == "PRESENT":
        ...
        rows.append(stopped)
        # A failed action remains a failure. Post-state is observed only after
        # a successful, bounded, ownership-scoped teardown command.
        success(stopped)
        probes = capture_group(recorder, ("server_container_probe", "server_process_probe"), "window", window_id)
        rows.extend(probes)
...
def window(recorder, window_id, speed):
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        ...
        # Reserve the second worker immediately so thread dispatch latency is
        # not added after the five-second deadline. The raw start timestamp
        # remains authoritative and the accepted 0.250-second bound is unchanged.
        def capture_ping():
...
def cleanup(recorder, before, *, require_complete=False):
    ...
    if current["containers"]:
        # cleanup_cycle validates ownership before this fixed-topology action;
        # no foreign or replacement container is eligible for destruction.
        from src.orchestration.x6_r1_4_1_observations import validate_owned_cleanup_state
    ...
    cycle = cleanup_cycle(before_rows, action, after_rows, initial=before, deployment=deployment,
                          require_complete=require_complete, run_id=auth["run_id"],
                          output_root=str(recorder.root), index=len(journal["cycles"]) + 1)
...
def execute(auth_path, root, run_id, *, simulation=False):
    ...
    source = identity()
    auth = load(auth_path)
    # Reject an already-invalid record without spending it. This preliminary
    # observation is not the permission to enter the lifecycle.
    ...
    # Approved coordination exception: this host-local lock may be acquired or
    # created before consumption solely to exclude another x6r1 controller.  It
    # grants no deployment, traffic, measurement, or qualification authority.
    topology_lock = acquire_topology_lock()
    try:
        reserve(
            root,
            auth,
            admission_validator=lambda observation: validate_new_attempt(
                auth,
                root=root,
                run_id=run_id,
...
def main():
    parser = argparse.ArgumentParser(description="R1.4 separately authorized baseline-only successor")
