...
from src.orchestration.x6_r1_5_contract import canonical, canonical_location, current_boot_id, digest, load, require
...
def advance_clock(nanoseconds):
    global _CLOCK_NS
    require(os.environ.get("X6_R1_5_CONTROLLED_CLOCK") == "1", "controlled clock is source-test only")
    with _CLOCK_LOCK:
...
def reservation_path(root, auth):
    return canonical_location(str(root)).parent / ".x6-r1-5-consumption" / (auth["authorization_id"] + ".json")
...
class Recorder:
    ...
    def capture(self, name, phase, *, window=None, started_event=None):
        ...
        if self.simulation and os.environ.get("X6_R1_5_CONTROLLED_CLOCK") == "1":
