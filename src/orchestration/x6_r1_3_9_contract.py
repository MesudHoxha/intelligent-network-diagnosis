"""H/I successor contract; no authorization artifact or runtime side effects."""
...
PREDECESSOR = "099b199f5b6bb72dd18060a3e0e168aa00a8178c"
RELEASE = "X6_R1_3_9_CLEANUP_AND_TEARDOWN_OBSERVATION_CORRECTION"
...
def identity(root=ROOT):
    ...
    paths = sorted(set(git("ls-files").splitlines()) | {
        str(p.relative_to(root)) for pattern in ("src/orchestration/x6_r1_3_9*.py", "src/expansion/x6_r1_3_9*.py", "plans/expansion/X6_R1_3_9*.json", "tests/fixtures/x6_r1_3_9_external_stub.py") for p in root.glob(pattern)
    })
