...
from src.orchestration.x6_r1_5_1_contract import RELEASE, canonical_location, identity, load, require, validate_committed_attempt, validate_partial_attempt
from src.orchestration.x6_r1_5_1_durable import Recorder, acquire_topology_lock, alive, process_identity, records, reservation_path, save, stamp
from src.orchestration.x6_r1_5_1_observations import catalog, failed_command_orders
from src.orchestration.x6_r1_5_1_production_path import assert_external_simulation, cleanup, restore, terminal
...
def recover(root, *, simulation=False):
    ...
    try:
        ...
        try:
            ...
            raise
        if old and old["status"] in {"F1_SOURCE_CONTRACT_COMPLETE_REPLAY_REQUIRED", "F1_SOURCE_CONTRACT_COMPLETE_REPLAY_VERIFIED", "F1_DIAGNOSTIC_ABSTENTION_RESTORATION_COMPLETE_REPLAY_REQUIRED", "F1_DIAGNOSTIC_ABSTENTION_RESTORATION_COMPLETE_REPLAY_VERIFIED"}:
            from src.expansion.x6_r1_5_1_materialized_verifier import verify
            verify(root, source_identity=source, simulation=simulation)
            status = ("F1_DIAGNOSTIC_ABSTENTION_RESTORATION_COMPLETE_REPLAY_VERIFIED"
                      if old["status"].startswith("F1_DIAGNOSTIC_ABSTENTION") else
                      "F1_SOURCE_CONTRACT_COMPLETE_REPLAY_VERIFIED")
            terminal(root, status, old.get("detail", ""))
        elif old:
            from src.expansion.x6_r1_5_1_materialized_verifier import verify
            if old["status"] == "CLEANUP_FAILED":
                terminal(root, "F1_ATTEMPT_FAILED_RECOVERY_CLEANUP_COMPLETE", "original cleanup failed; standalone recovery later established owned cleanup")
            verify(root, source_identity=source, simulation=simulation, permit_interrupted=True)
