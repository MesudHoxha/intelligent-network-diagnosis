...
class X2MissingDefaultRouteExperimentError(RuntimeError):
    """Raised when the isolated X2-R3 lifecycle cannot be accepted."""


BASELINE_TIMEOUT_SECONDS = 120.0
BaselineValidator = Callable[[Path], dict[str, object]]
FaultMutator = Callable[..., dict[str, object]]
EvidenceCollector = Callable[..., dict[str, object]]
VectorBuilder = Callable[..., dict[str, object]]
...
def _restoration_confirmed(mutation_directory: Path) -> bool:
    ...
    return path.is_file() and load_json_object(path).get("status") == (
        "RESTORATION_CONFIRMED"
    )


def recover_x2_r3_experiment(
    scenario_path: Path,
    experiment_directory: Path,
    baseline_validator_path: Path,
    *,
    baseline_validator: BaselineValidator = run_baseline_validator,
    fault_restorer: FaultMutator = restore_missing_default_route,
) -> dict[str, object]:
    ...
    if not experiment_directory.is_dir() or not _recovery_exists(mutation_directory):
        raise X2MissingDefaultRouteExperimentError(
            "X2-R3 recovery requires an existing matching recovery journal."
        )
    restoration = fault_restorer(scenario_path, mutation_directory)
    if restoration.get("status") != "RESTORATION_CONFIRMED":
        raise X2MissingDefaultRouteExperimentError("X2-R3 recovery was not confirmed.")
    ...
    if baseline.get("return_code") != 0:
        raise X2MissingDefaultRouteExperimentError(
            "X2-R3 restoration completed but baseline recovery failed."
        )
    result = {
        "schema_version": 1,
        "release_id": "X2_R3_MISSING_DEFAULT_ROUTE",
...
def run_x2_r3_experiment(
    scenario_path: Path,
    output_root: Path,
    baseline_validator_path: Path,
    *,
    baseline_validator: BaselineValidator = run_baseline_validator,
    fault_injector: FaultMutator = inject_missing_default_route,
    fault_restorer: FaultMutator = restore_missing_default_route,
    evidence_collector: EvidenceCollector = collect_missing_default_route_evidence_v4,
    vector_builder: VectorBuilder = build_feature_vector_v2,
    rule_engine: RuleEngine = diagnose_addressing_v2,
    ...
    try:
        ...
        if not baseline_before_passed:
            raise X2MissingDefaultRouteExperimentError(
                "X2-R3 baseline validation failed before mutation."
            )
        ...
        diagnosis = rule_engine(
            ...
            affected_resource=binding.source_interface,
        )
        write_json_atomic(
            experiment_directory / "diagnosis/diagnosis_result_v2.json",
            diagnosis,
        )
        if (
            diagnosis.get("status") != "diagnosed"
            or not isinstance(diagnosis.get("prediction"), dict)
            or diagnosis["prediction"].get("fault_type") != "missing_default_route"
        ):
            raise X2MissingDefaultRouteExperimentError(
                "X2-R3 exact Missing Default Route rule did not produce the expected diagnosis."
            )
    ...
    if _recovery_exists(mutation_directory):
        try:
            ...
            if restoration.get("status") != "RESTORATION_CONFIRMED":
                raise X2MissingDefaultRouteExperimentError(
                    "X2-R3 restoration record is not confirmed."
                )
        except BaseException as error:
    ...
    if baseline_before_passed and restoration_error is None:
        ...
        if baseline_after.get("return_code") != 0:
            restoration_error = X2MissingDefaultRouteExperimentError(
                "X2-R3 baseline validation failed after restoration."
            )
    ...
    if final_error is not None:
        ...
        write_json_atomic(experiment_directory / "manifest.json", manifest)
        raise X2MissingDefaultRouteExperimentError(
            f"X2-R3 experiment failed: {final_error}. Artifacts: "
            f"{experiment_directory}"
        ) from final_error

    if not _restoration_confirmed(mutation_directory):
        raise X2MissingDefaultRouteExperimentError(
            "X2-R3 completed without a confirmed restoration record."
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the isolated X2-R3 Missing Default Route lifecycle."
    )
    parser.add_argument("--scenario", type=Path, required=True)
    parser.add_argument("--output-root", type=Path, default=Path("data/raw/x2_r3"))
...
def main() -> int:
    ...
    try:
        if arguments.recover_experiment_directory is not None:
            result = recover_x2_r3_experiment(
                arguments.scenario,
    ...
    except (X2AddressingError, X2MissingDefaultRouteExperimentError, OSError) as error:
        print(f"[ERROR] {error}")
        return 1
