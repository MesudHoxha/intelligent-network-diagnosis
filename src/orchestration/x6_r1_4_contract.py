"""Prospective R1.4 baseline-only authorization contract.

The local operator account is the trust boundary.  The canonical SHA-256 binds
content but is not cryptographic issuer authentication.  A concrete record is
external, user-reviewed, and absent from this source release.
"""
...
PREDECESSOR = "2dde4972cd2228a38da7dee12a31506673e45b12"
RELEASE = "X6_R1_4_BASELINE_ONLY_RUNTIME_AUTHORIZATION_PREPARATION"
AUTHORIZATION_RELEASE = "X6_R1_4_BASELINE_ONLY_RUNTIME_AUTHORIZATION"
...
def identity(root=ROOT):
    ...
    paths = sorted(set(git("ls-files").splitlines()) | {
        str(p.relative_to(root)) for pattern in ("src/orchestration/x6_r1_4*.py", "src/expansion/x6_r1_4*.py", "plans/expansion/X6_R1_4*.json", "tests/fixtures/x6_r1_4_external_stub.py") for p in root.glob(pattern)
    })
...
def validate_authorization(value, *, root, run_id, source_identity, now_ns, simulation=False):
    ...
    require(value["schema_version"] == 3 and value["record_kind"] == "CONCRETE_AUTHORIZATION" and value["release_id"] == AUTHORIZATION_RELEASE, "authorization release")
