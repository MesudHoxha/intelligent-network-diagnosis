"""Phase-aware command catalog and raw-only F1 reconstruction."""
...
from src.collection.x6_performance_collector import derive_window, exact_fault_hierarchy, exact_noqueue, validate_speed_pair
from src.orchestration.x6_r1_5_1_contract import ROOT, CONTEXT, TOPOLOGY, IMAGE, NODES, canonical, digest, require
...
def six_place(value, label):
    try:
        return finite_decimal(value, label).quantize(SIX_PLACES, rounding=ROUND_HALF_EVEN)
    except InvalidOperation:
        require(False, "numeric range " + label)


def aggregate_fault_windows(windows, pooled_rtt_replies):
    """Aggregate normalized F01-F03 values, then normalize each aggregate once."""
    require(len(windows) == 3, "fault aggregation requires exactly three windows")
    numeric = ("packet_loss_ratio", "throughput_mbps", "interface_utilization_ratio")
    for row in windows:
        require(set(row["measurements"]) == {
            "packet_loss_ratio", "round_trip_latency_ms_p95", "throughput_mbps",
            "interface_utilization_ratio", "queue_drop_count", "rate_limit_detected",
        }, "fault measurement inventory")
        for name in numeric:
            require(finite_decimal(row["measurements"][name], name) == six_place(row["measurements"][name], name), "fault window is not six-place normalized")
        require(type(row["measurements"]["queue_drop_count"]) is int and row["measurements"]["queue_drop_count"] >= 0, "fault queue type")
        require(type(row["measurements"]["rate_limit_detected"]) is bool, "fault rate-limit type")
    losses = [finite_decimal(row["measurements"]["packet_loss_ratio"], "packet_loss_ratio") for row in windows]
    throughputs = sorted(finite_decimal(row["measurements"]["throughput_mbps"], "throughput_mbps") for row in windows)
    utilizations = sorted(finite_decimal(row["measurements"]["interface_utilization_ratio"], "interface_utilization_ratio") for row in windows)
    replies = sorted(finite_decimal(value, "fault RTT reply") for value in pooled_rtt_replies)
    require(replies, "fault RTT samples absent")
    latency = replies[math.ceil(Decimal("0.95") * len(replies)) - 1]
    values = {
        "packet_loss_ratio": six_place(sum(losses) / Decimal(3), "packet_loss_ratio aggregate"),
        "round_trip_latency_ms_p95": six_place(latency, "round_trip_latency_ms_p95 aggregate"),
        "throughput_mbps": six_place(throughputs[1], "throughput_mbps aggregate"),
        "interface_utilization_ratio": six_place(utilizations[1], "interface_utilization_ratio aggregate"),
        "queue_drop_count": sum(row["measurements"]["queue_drop_count"] for row in windows),
        "rate_limit_detected": any(row["measurements"]["rate_limit_detected"] for row in windows),
    }
    return {
        name: {"availability": "observed", "value": value if isinstance(value, (int, bool)) else float(value)}
...
def catalog():
    ...
    commands = {
        ...
        "mutation_root": (context["qdisc"]["forward_commands"][0], context["qdisc"]["command_timeout_seconds"]),
        "mutation_child": (context["qdisc"]["forward_commands"][1], context["qdisc"]["command_timeout_seconds"]),
        "mutation_restore": (context["qdisc"]["recovery_command"], context["qdisc"]["command_timeout_seconds"]),
...
def controls(rows, *, phase="baseline"):
    ...
    if phase == "fault":
        require(exact_fault_hierarchy(rows[2]), "fault qdisc hierarchy")
        try:
            filters = [json.loads(row["stdout"]) for row in rows[3:]]
        except json.JSONDecodeError:
            require(False, "fault filter state")
            filters = []
        require(filters == [[], []], "foreign fault filter state")
    else:
        require(exact_noqueue(rows[2], rows[3:]), "healthy qdisc/filter state")
    counters = [int(row["stdout"].strip()) for row in rows[:2]]
    require(all(x >= 0 for x in counters), "negative counter")
    return counters


def reconstruct_window(rows, speed, *, phase="baseline"):
    require(phase in {"baseline", "fault", "restored"}, "window phase")
    ...
    before_c, after_c = controls(before, phase=phase), controls(after, phase=phase)
    ...
    derived = derive_window(raw, phase=phase, speed_mbps=speed)
    ...
    for key, value in derived.items():
        ...
        measurements[key] = int(observed) if key == "queue_drop_count" else float(six_place(observed, key))
